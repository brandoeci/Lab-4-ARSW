...
**GET a autor inexistente (404):**

> **[PEGAR CAPTURA DE PANTALLA AQUÍ]**

**POST con validación fallida — campo vacío (400):**

> **[PEGAR CAPTURA DE PANTALLA AQUÍ]**

**POST a un blueprint duplicado (400):**
...
> 💡 **Para la sustentación:** cada código de error tiene su propio `@ExceptionHandler` en `ApiExceptionHandler` (`@RestControllerAdvice`). El controller (`BlueprintsAPIController`) no tiene ningún `try/catch` — las excepciones suben solas y este componente las traduce al código HTTP correcto. Esto es lo que el lab pide como "control de errores centralizado".
...
> 💡 **Para la sustentación:** la tabla `points` tiene una columna `seq` que no existe en el modelo de dominio (`Point` solo tiene `x` y `y`). Esa columna existe únicamente para preservar el **orden** de los puntos al leerlos de vuelta — sin ella, una consulta SQL no garantiza el orden de inserción, y el orden de los puntos de un blueprint sí importa (define la forma del dibujo).
...
> 💡 **Para la sustentación:** `IdentityFilter` necesitó la anotación `@Profile("!redundancy & !undersampling")` para no quedar activo al mismo tiempo que los otros dos filtros. Sin esto, Spring lanza un error de ambigüedad (`expected single matching bean but found 2`) porque `BlueprintsServices` solo puede recibir un único `BlueprintsFilter` por inyección de dependencias.
...
> 💡 **Para la sustentación:** Testcontainers levanta un contenedor de PostgreSQL **distinto** al de `docker-compose.yml` — es efímero, se crea y destruye automáticamente en cada ejecución de `mvn test`, así que las pruebas nunca dependen de datos previos ni contaminan la base de datos de desarrollo.
